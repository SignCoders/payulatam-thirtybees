...
if (Tools::strlen($description) > 255) {
...
<?php
if (_PS_VERSION_ < '1.5') {
    $response_url = 'https://' . htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8') .
...
$confirmation_url = 'https://' . htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8') .
...
<form class="md-form" id="payu_latam_form" name="payu_latam_form" method="post"
      action="<?php echo Tools::safeOutput($gateway_url); ?>">
    <input type="hidden" name="merchantId" id="merchantId"
           value="<?php echo Tools::safeOutput(Configuration::get('PAYU_LATAM_MERCHANT_ID')); ?>"/>
    <input type="hidden" name="referenceCode" id="referenceCode"
           value="<?php echo Tools::safeOutput((int)$cart->id); ?>"/>
    <input type="hidden" name="description" id="description"
           value="<?php echo Tools::safeOutput(trim($description, ',')); ?>"/>
    <input type="hidden" name="amount" id="amount"
           value="<?php echo Tools::safeOutput($cart->getordertotal(true)); ?>"/>
    ...
    <input type="hidden" name="accountId" id="accountId"
           value="<?php echo Tools::safeOutput(Configuration::get('PAYU_LATAM_ACCOUNT_ID')); ?>"/>
    ...
    <input type="hidden" name="extra1" id="extra1"
           value="<?php echo Tools::safeOutput('Prestashop ' . _PS_VERSION_); ?>"/>
    ...
    <input type="hidden" name="confirmationUrl" id="confirmationUrl"
           value="<?php echo Tools::safeOutput($confirmation_url); ?>"/>
    <input type="hidden" name="payerFullName" id="payerFullName"
           value="<?php echo Tools::safeOutput($customer->firstname . ' ' . $customer->lastname); ?>"/>
    <input type="hidden" name="billingAddress" id="billingAddress"
           value="<?php echo Tools::safeOutput($billing_address->address1); ?>"/>
    <input type="hidden" name="shippingAddress" id="shippingAddress"
           value="<?php echo Tools::safeOutput($delivery_address->address1); ?>"/>
    <input type="hidden" name="telephone" id="telephone"
           value="<?php echo Tools::safeOutput($billing_address->phone); ?>"/>
    <input type="hidden" name="billingCity" id="billingCity"
           value="<?php echo Tools::safeOutput($billing_address->city); ?>"/>
    <input type="hidden" name="shippingCity" id="shippingCity"
           value="<?php echo Tools::safeOutput($delivery_address->city); ?>"/>
    <input type="hidden" name="billingCountry" id="billingCountry"
           value="<?php echo Tools::safeOutput($billing_address->country->iso_code); ?>"/>
    <input type="hidden" name="shippingCountry" id="shippingCountry"
           value="<?php echo Tools::safeOutput($delivery_address->country->iso_code); ?>"/>
</form>
